...
for i in range(10):
    ...
    df_train = df_train[["Pcorrectforproblem", "Pmedian", "p_prior_correct", "p_prior_completed", "prior_attempts",
                         "pProblemSemanticError", "Prob_synt"
        , "pSubjectSyntaxErrors", "pSubjectSemanticErrors", "Correct", "StartOrder", "ProblemID", "Code"]]
    df_test = df_test[["Pcorrectforproblem", "Pmedian", "p_prior_correct", "p_prior_completed", "prior_attempts",
                       "pProblemSemanticError", "Prob_synt"
        , "pSubjectSyntaxErrors", "pSubjectSemanticErrors", "Correct", "StartOrder", "ProblemID", "Code"]]
    df_test_syntax_error = df_test_syntax_error[
        ["Pcorrectforproblem", "Pmedian", "p_prior_correct", "p_prior_completed", "prior_attempts",
         "pProblemSemanticError", "Prob_synt"
            , "pSubjectSyntaxErrors", "pSubjectSemanticErrors", "Correct", "StartOrder", "ProblemID", "Code"]]
...
d = {"accuracy": [mean(accuracy_list)], "f1_score": mean(f1_score_list), "precision_score": mean(precision_score_list),
     "kappa_score": mean(kappa_score_list), "recall_score": mean(recall_score_list)}
...
df_accuracy = df_accuracy.assign(f1_score=(2 * df_accuracy["precision"] * df_accuracy["recall"]) / (
            df_accuracy["precision"] + df_accuracy["recall"]))
df_accuracy = df_accuracy.assign(
    pe=(df_accuracy["pcorrect"] * df_accuracy["ppredicted"]) + (1 - df_accuracy["pcorrect"]) * (
                1 - df_accuracy["ppredicted"]))
